...
export class SnowflakeDriver extends BaseDriver implements DriverInterface {
  ...
  public constructor(
    ...
    this.config = {
      ...
      caseSensitiveIdentifiers: getEnv('caseSensitiveIdentifiers', { dataSource }),
  ...
  protected async initConnection() {
    try {
      ...
      await this.execute(connection, `ALTER SESSION SET QUOTED_IDENTIFIRS_IGNORE_CASE = ${this.config.caseSensitiveIdentifiers}`, [], false);
